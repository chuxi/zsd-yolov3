...
def yolo_body(inputs, num_seen, num_anchors):
    ...
    x, y1 = make_last_layers(darknet.output, 512, num_anchors * (5 + num_seen))

    x = compose(DarknetConv2D_BN_Leaky(256, (1, 1)),
                UpSampling2D(2))(x)
    x = Concatenate()([x, darknet.layers[152].output])
    x, y2 = make_last_layers(x, 256, num_anchors * (5 + num_seen))

    x = compose(DarknetConv2D_BN_Leaky(128, (1, 1)),
                UpSampling2D(2))(x)
    x = Concatenate()([x, darknet.layers[92].output])
    x, y3 = make_last_layers(x, 128, num_anchors * (5 + num_seen))
...
def yolo_head(feats, anchors, num_seen, input_shape, calc_loss=False):
    ...
    box_attribute = K.reshape(feats, [-1, grid_shape[0], grid_shape[1], num_seen, 300])
    feats = K.reshape(feats, [-1, grid_shape[0], grid_shape[1], num_anchors, 5 + num_seen])
...
def yolo_loss(args, anchors, num_seen, ignore_thresh=.5):
    ...
    for l in range(num_layers):
        ...
        _, ignore_mask = tf.control_flow_ops.while_loop(lambda b, *arg: b < m, loop_body, [0, ignore_mask])
        ignore_mask = ignore_mask.stack()
        ignore_mask = K.expand_dims(ignore_mask, -1)

        xy_loss = object_mask * box_loss_scale * K.binary_crossentropy(raw_true_xy, raw_pred[...,0:2], from_logits=True)
        wh_loss = object_mask * box_loss_scale * 0.5 * K.square(raw_true_wh-raw_pred[...,2:4])
        object_loss = object_mask * K.binary_crossentropy(object_mask, raw_pred[...,4:5], from_logits=True)+ \
                          (1-object_mask) * K.binary_crossentropy(object_mask, raw_pred[...,4:5], from_logits=True) * ignore_mask
        attribute_loss = object_mask * K.binary_crossentropy(true_class_probs, raw_pred[...,5:], from_logits=True)
